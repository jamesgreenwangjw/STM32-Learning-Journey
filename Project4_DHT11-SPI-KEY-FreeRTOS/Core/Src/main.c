...
/* USER CODE BEGIN PV */
volatile uint16_t adc_buf[MAX_LEN];
extern osSemaphoreId_t SemRestartHandle;
extern osSemaphoreId_t SemStopHandle;
	...
	if(GPIO_Pin == KEY1_Pin)
		{
			HAL_TIM_PWM_Stop(&htim3 , TIM_CHANNEL_2);
			HAL_TIM_PWM_Stop(&htim8 , TIM_CHANNEL_1);
			osSemaphoreRelease(SemStopHandle);
		}
	else if(GPIO_Pin == KEY2_Pin)
	{
		osSemaphoreRelease(SemRestartHandle);
	}
}
*/
